...
To nest one tag under another, use a forward slash between words, like: . In the Sidebar, a folding button appears next to the top-level tag which reveals its nested tags.
...
### Smart Search
Like most search engines, Bear supports a variety of **search operators** to filter your results. For example, you can search for a specific phrase by, exclude keywords by adding a minus symbol (-) before them, and more. [Learn about Bear search operators](https://bear.app/faq/).
